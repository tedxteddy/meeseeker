...
import { searchJobs, searchJobsApify, searchJobsYC, getApiKeyStatus } from '../lib/api'
...
export default function JobsView({ jobs, applications, onAddApplication, onDeleteJob, onJobImported }: JobsViewProps) {
  ...
  async function handleSearch(e: FormEvent) {
    ...
    try {
      const searchFn = searchSource === 'apify' ? searchJobsApify : searchSource === 'yc' ? searchJobsYC : searchJobs
    ...
    <>
      <form onSubmit={handleSearch} className="controls">
        <select className="filter-select" value={searchSource} onChange={e => setSearchSource(e.target.value)}>
          ...
          <option value="yc">YC Jobs (free, no key)</option>
        </select>
      ...
      {searchResults.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div className="stats-bar">
            <span>{searchResults.length} results from <strong>{searchSource === 'jsearch' ? 'JSearch' : searchSource === 'apify' ? 'Apify' : 'YC Jobs'}</strong></span>
          </div>
          <div className="job-grid">
            {searchResults.slice(0, 10).map((result, i) => (
              <div key={i} className="job-card">
                <div className="job-main">
                  <div className="job-title">{result.job_title}</div>
                ...
                {isSelected && (
                  <div className="detail-panel open">
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ background: 'var(--surface)', borderRadius: 8, padding: 12, marginBottom: 12, border: '1px solid var(--border)' }}>
                          <h4 style={{ fontSize: 12, margin: '0 0 8px 0' }}>Match Breakdown</h4>
                          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                            <div style={{ flex: 1, textAlign: 'center', padding: '6px 4px', borderRadius: 6, background: breakdown.overall >= 70 ? 'rgba(34,197,94,0.1)' : breakdown.overall >= 40 ? 'rgba(255,159,28,0.1)' : 'rgba(239,68,68,0.1)' }}>
                            ...
                            <div style={{ flex: 1, textAlign: 'center', padding: '6px 4px', borderRadius: 6, background: 'rgba(0,0,0,0.03)' }}>
                          ...
                          {breakdown.skillsToAdd.length > 0 && (
                            <div style={{ marginTop: 8, padding: 8, background: 'rgba(255,159,28,0.08)', borderRadius: 6 }}>
                    ...
                    <div className="detail-actions">
                      {job.source_url && (
                        <a className="btn btn-outline btn-sm" href={job.source_url} target="_blank" rel="noreferrer">
                          Apply
                        </a>
                      )}
